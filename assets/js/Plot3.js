...
function LineGraph (div, settings) {
	...
	this.plot = function () {
		if (this.data.pts && this.data.pts.length) {
			this.drawing = new LineGraphDrawing({'canvas':this.canvas, 'data':this.data, 'extremes':this.extremes, 'edge':{'left':0.1, 'bottom':0.1, 'top':0.1, 'right':0.1}, 'fix':this.settings.fix});
			this.drawing.draw();
			var this_obj = this;
...
function LineGraphDrawing (settings) {
	...
	this._draw_point = function (pt) {
		...
		var text = {
			'x':((this.settings.fix && this.settings.fix.x)?(prj_pt.x).toFixed(this.settings.fix.x).toString() : prj_pt.x.toString()) + ' ' + units.x,
			'y':((this.settings.fix && this.settings.fix.y)?(prj_pt.y).toFixed(this.settings.fix.y).toString() : prj_pt.y.toString()) + ' ' + units.y
		}
		this.context.fillStyle = this.settings.metricFillStyle;
		this.context.fillText(text.x, px_pt.x, origin.y + dimensions.height);
		this.context.fillText(text.y, origin.x, px_pt.y);
	}
