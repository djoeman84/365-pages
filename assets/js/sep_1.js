...
function Swirl (drawing) {
	...
	this.tail_len = 50 + 1;
	var len_to_rad = 0.2;
	this.points = [new Point(this.tail_len*len_to_rad, this.leader.x, this.leader.y, this.drawing)];
	for (var i = this.tail_len - 1; i > 0; i--) {
		this.points.push(new Point(i*len_to_rad, this.leader.x, this.leader.y, this.drawing));
	};
	...
	this.update_leader_pos = function () {
		...
		this.points[0].y =  this.drawing.center.y + (Math.cos(this.rad))*this.deviation.y;
	}

	this.update_points = function () {
		do {
			this.approach_rate += (Math.random() * 0.4 -0.2);
		} while(this.approach_rate <= 1);
